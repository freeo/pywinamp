...
class Winamp(object):
    ...
    # gets current position in playlist
    IPC_GETLISTPOS = 125
    ...
    def __readStringFromMemory(self, address, isUnicode = False):
        ...
        """Note: this is quite an ugly hack, because we assume the string will
        have a maximum size of MAX_PATH (=260) and that we're not in an end of
        a page.
    ...
    def dequeueFile(self, index):
        '''Dequeues (deletes) a title from playlist.
    ...
    def query(self, queryString, queryType = ML_IPC_DB_RUNQUERY):
        ...
        buf = create_string_buffer(sizeof(self.itemRecord) *
                receivedQuery.itemRecordList.Size)
        ...
        # free results
        win32api.SendMessage(self.__mediaLibraryHWND, self.WM_ML_IPC,
                queryStructAddr, self.ML_IPC_DB_FREEQUERYRESULTS)

        return items

    def queryAsKeyword(self, queryString):
        """Queries Winamp's media library and returns a list of items matching
        the query.

        The query should be a keyword, like 'alice' (then the query is then
        treated as a string query).  This makes Winamp search the requested
        keyword in every data field in the media library database (such as
    ...
    def __copyDataToWinamp(self, data):
        if type(data) is str:
            ...
            lpAddress = windll.kernel32.VirtualAllocEx(self.__hProcess, None,
                    sizeof(dataToCopy), win32con.MEM_COMMIT,
                    win32con.PAGE_READWRITE)
            # write data to Winamp's memory
            windll.kernel32.WriteProcessMemory(self.__hProcess, lpAddress,
                    addressof(dataToCopy), sizeof(dataToCopy), None)
    ...
    def __readDataFromWinamp(self, address, ctypesType):
        if ctypesType is c_char_p:
            ...
            bytesRead = c_ulong(0)
            if windll.kernel32.ReadProcessMemory(self.__hProcess, address,
                    buffer, sizeof(buffer), byref(bytesRead)) == 0:
                # we're in a new page
...
def printMediaLibraryItem(item):
    print('''Filename: %s\nTrack: %s, Album: %s, Artist: %s\nComment: %s,
                Genre: %s''' % (item.filename, item.track, item.album,
                    item.artist, item.comment, item.genre))
